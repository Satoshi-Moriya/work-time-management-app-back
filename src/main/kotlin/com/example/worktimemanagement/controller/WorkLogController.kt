...
class WorkLogController(val workLogService: WorkLogService) {
    ...
    fun getSelectedMonthlyWorkLog(@PathVariable @NotNull userId: Int, @RequestParam("from") fromDate: String, @RequestParam("to") toDate: String): List<WorkLog> {
        return workLogService.findByBetweenYearAndMonth(userId, fromDate, toDate)
    }

    @PostMapping("/work-log")
    @ResponseStatus(HttpStatus.CREATED)
    fun save(@RequestBody @Validated workLogBody: WorkLog): List<WorkLog> {
